...
**核心原则验证**:
- [ ] JetBrains 插件开发规范: 是否使用官方 API 和推荐模式? 是否会通过 Plugin Verifier?
- [ ] Java 业界规范: 代码风格、命名、异常处理是否符合标准?
- [ ] 单元测试覆盖率: 测试计划是否能达到 60% 覆盖率? 关键逻辑是否有更高覆盖?
- [ ] codei18n CLI 封装: 进程调用、错误处理、版本兼容性是否考虑周全?
- [ ] 用户体验: UI 响应性、进度反馈、错误消息是否用户友好?
- [ ] 向后兼容性: 支持的 IDE 版本范围是否明确? 是否测试边界版本?
- [ ] 可观测性: 日志级别和诊断信息是否充分?

**技术约束验证**:
- [ ] 技术栈: Java 8+, Gradle, JUnit 5 是否满足?
- [ ] 性能: UI 响应 < 100ms, 异步执行长操作是否考虑?
- [ ] 安全: 输入验证、敏感信息处理是否安全?

**如有违规**: 在"复杂度跟踪"部分记录并证明必要性.
